...
with tab1:
    if df_record.empty:
        ...
        c4.metric("💱 USD/TWD 匯率", f"{usd_rate:.2f}")
        ...
        with chart_col2:
            ...
            plot_df = portfolio[portfolio["幣別"] == "TWD"] if view_option == "台股組合" else portfolio[portfolio["幣別"] == "USD"] if view_option == "美股組合" else portfolio
...
with tab2:
    if not df_record.empty:
        st.subheader("💡 個股技術健診")
        target = st.selectbox("選擇要分析的標的：", portfolio["股票代號"].tolist())
        if target:
            with st.spinner(f"正在分析 {target}..."):
                ...
                if err: st.error(err)
                    ...
                    c2.metric("半年最高", f"{res['high_6m']:.2f}")
                    c3.metric("半年最低", f"{res['low_6m']:.2f}")
                    c4.metric("RSI 指標", f"{res['rsi']:.1f}")
                    st.divider()
                    st.markdown(f"#### 趨勢： **{res['trend']}**")
                    st.success(f"**操作建議**：:{res['advice_color']}[{res['advice']}]")
