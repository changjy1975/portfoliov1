...
if not df_raw.empty:
    ...
    with tab2:
        ...
        if not df_t.empty:
            ...
            fig.add_trace(go.Scatter(x=b.index, y=b['Low']*0.98, mode='markers', marker=dict(symbol='triangle-up', size=14, color='lime', line=dict(width=1, color='white')), name='買'), row=1, col=1)
            fig.add_trace(go.Scatter(x=s.index, y=s['High']*1.02, mode='markers', marker=dict(symbol='triangle-down', size=14, color='red', line=dict(width=1, color='white')), name='賣'), row=1, col=1)
            ...
            fig.add_trace(go.Scatter(x=df_t.index, y=df_t['K'], line=dict(color='white')), row=2, col=1); fig.add_trace(go.Scatter(x=df_t.index, y=df_t['D'], line=dict(color='yellow')), row=2, col=1)
